...
def train(args, data, show_loss, show_topk, log_dir):
    ...
    with tf.Session(config=config) as sess,\
        open(log_dir + 'result_' + str(args.epochs) + '_' + str(int(time.time())) + '.txt', 'w') as f_result:
        ...
        for step in range(args.epochs):
            ...
            while start < train_data.shape[0]:
                _, loss = model.train_rs(sess, get_feed_dict_for_dmf(model, train_data, start, start + args.batch_size))
...
def get_feed_dict_for_dmf(model, data, start, end):
